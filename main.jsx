        ...
        {nav === "panique" && (() => {
          ...
          return (
            <div>
              ...
              {panique.solde !== "" && (
                <>
                  {/* ─── Détail du calcul : TOUT visible, rien de caché ─── */}
                  <div style={{ ...S.card, marginTop: 14 }}>
                    ...
                    <div style={S.paniqueResult}>
                      ...
                      <span style={{ ...S.paniqueResultValue, fontSize: 44, color: apresReserve > 0 ? c.text : "#A32D2D" }}>{formatEUR(Math.max(0, apresReserve))}</span>
                  ...
                  {(() => {
                    ...
                    const risqueLabel = { vert: "Faible", orange: "Modéré", rouge: "Élevé" }[score];
                    return (
                      <div style={{ ...S.card, marginTop: 14, background: c.bg, border: `1px solid ${c.dot}` }}>
                        <div style={S.cardTitle}>{c.emoji} Êtes-vous en sécurité financière ?</div>
                        {moisSurvie !== null && (
                          <div style={S.checkRow}>
                            <span style={{ color: moisSurvie >= 3 ? "#1D9E75" : "#A32D2D" }}>{moisSurvie >= 3 ? "🟢" : "🔴"}</span>
                            <span style={{ fontSize: 13, color: c.text }}>Vous pouvez tenir <strong>{moisSurvie} mois</strong> sans revenus</span>
                          </div>
                        )}
                        <div style={S.checkRow}>
                          <span>{urssafOk ? "🟢" : "🔴"}</span>
                          <span style={{ fontSize: 13, color: c.text }}>{urssafOk ? "Votre URSSAF est provisionnée" : `URSSAF non provisionnée (${formatEUR(Math.max(0, urssaf - soldeNum))} manquants)`}</span>
                        </div>
                        <div style={S.checkRow}>
                          <span>{reserveOk ? "🟢" : "🔴"}</span>
                          <span style={{ fontSize: 13, color: c.text }}>{reserveOk ? "Votre réserve de sécurité est atteinte" : "Réserve de sécurité insuffisante"}</span>
                        </div>
                        <div style={{ ...S.checkRow, marginTop: 4, paddingTop: 10, borderTop: `1px solid ${c.dot}40` }}>
                          <span style={{ fontSize: 13, fontWeight: 700, color: c.text }}>Risque global : {risqueLabel}</span>
                        </div>
                      </div>
                    );
                  })()}
